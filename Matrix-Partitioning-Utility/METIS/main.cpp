...
int main(int argc, char *argv[]){
    ...
    if(argc < 2){
        ...
        for(int j = 0; j < arguments.size(); j++){
            ...
            if(arguments[j] == "-s" || arguments[j] == "--save"){
                savePartitionToFile = true;
            }
        }
    }
...
//    }
    ...
    if (mm_is_complex(matcode)) is_complex = true;
    ...
    double complex_val_second;
    for (int i=0; i<nz; i++)
    {
        int a, b;
        if(is_complex) fscanf(f, "%d %d %lg %lg\n", &a, &b, &val[i], &complex_val_second);
    ...
    if (savePartitionToFile) {
      //adjusting the output file.
      std::string partVectorName = "output/"+fileName +"_metis_"+objective_name+ "_part" + std::to_string(nParts) + ".txt";
      ...
      std::string statusFileName = "output/"+fileName +"_metis_"+objective_name+ "_part" + std::to_string(nParts) + "_info" + ".txt";
...
//        std::cout<<outputNameString<<((double)elapsedTimeForPartitioning/1000)<<" seconds"<<std::endl;
        std::cout << "Dataset: " << fileName << " PartitionTime: " << ((double)elapsedTimeForPartitioning/1000) <<" seconds."<<std::endl;
    }
