...
int main(int argc, char* argv[]) {
  ...
  if (argc < 4 && (argc == 4 || (argc == 5 && std::string("-s") != argv[4]) || (argc == 6 && std::string("-c") != argv[4]))) {
    std::cerr << "Usage: reorder GRAPH_FILE REORDER_FILE COMMUNITY_FILE [-s] [-c]\n"
              << "  -s    Save the result to a file\n"
  ...
  const bool save_to_file = argc == 5;
  const bool commode = argc == 6;
